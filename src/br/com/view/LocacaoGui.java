 ...
 */
public class LocacaoGui extends javax.swing.JFrame {

    private LocacaoTableModel locacao;

    public LocacaoGui() {
        initComponents();
        List<Locacao> lista = new ArrayList<Locacao>();
        LocacaoController a = new LocacaoController();
        lista = a.listarLocacao();

        //cria o modelo de Produto      
        locacao = new LocacaoTableModel(lista);

        //atribui o modelo à tabela
        jTable1.setModel(locacao);
    ...
    private void btInserirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btInserirActionPerformed
        InserirLocacaoGui im = new InserirLocacaoGui(locacao);
        im.setVisible(true);
    ...
    private void btRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btRemoverActionPerformed
        ...
        if (selecionado >= 0) {
            Integer i = (Integer) locacao.getValueAt(selecionado, 0);
            LocacaoController mc = new LocacaoController();
            if (mc.excluir(i)){
            locacao.removeLocacao(selecionado);
            }
        } else {
    ...
    private void btEditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btEditarActionPerformed
        ...
        if (selecionado >= 0) {
            Integer cdLocacao = (Integer) locacao.getValueAt(selecionado, 0);
            AtualizarLocacaoGui am = new AtualizarLocacaoGui(selecionado, cdLocacao, locacao);
            am.setVisible(true);
        } else {
    ...
    private void efetuaPesquisa() {
        ...
        List<Locacao> lista = new ArrayList<Locacao>();
        LocacaoController a = new LocacaoController();
        lista = a.listarLocacaosByDescricao(descricao);
        locacao.limpar();
